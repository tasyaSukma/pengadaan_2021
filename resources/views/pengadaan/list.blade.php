                  ...
                  <div class="card-body table-responsive p-0">
                    <table style="text-align: center;" class="table table-hover text-nowrap">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>{{$pgd->nama_pengadaan}}</td>
                          <td><a target="_blank" href="{{$pgd->deskripsi}}"><button class="btn btn-primary">Lihat Detail</button></a></td>
                          <td>
                            @if($pgd->gambar !="-")
                            <img style="width:15%;" src="{{asset(Storage::url($pgd->gambar))}}">
                            <hr>
                            <a class="konfirmasi" href="/hapusGambar/{{$pgd->id_pengadaan}}"><button class="btn btn-danger"><i class="fas fa-trash"></i> Hapus</button>
                              @endif
                              @if($pgd->gambar == "-")
                              <hr>
                              <form method="post" action="/uploadGambar" enctype="multipart/form-data" role="form">
                                {{csrf_field()}}
                                <input type="hidden" name="id_pengadaan" id="id_pengadaan" value="{{$pgd->id_pengadaan}}">
                            ...
                            <td><span class="tag tag-success">{{number_format($pgd->anggaran,0,",",".")}}</span></td>
                            <td>
                              <a class="konfirmasi" href="/hapusPengadaan/{{$pgd->id_pengadaan}}">
                                <button type="button" class="btn btn-danger"><i class="fas fa-trash">Hapus</i></button>
                              </a>
                              <button class="btn btn-secondary ubah" data-toggle="modal" data-target="#ubahModal" data-nama_pengadaan="{{$pgd->nama_pengadaan}}" data-deskripsi="{{$pgd->deskripsi}}" data-anggaran="{{$pgd->anggaran}}" data-id_pengadaan="{{$pgd->id_pengadaan}}"><i class="fas fa-edit">Ubah</i></button>
                            </td>
                          </tr>
                          @endforeach
                        </tbody>
